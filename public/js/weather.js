...
const weather = async () => {
  try {
    ...
    if (response.status != 200) {
      throw new Error(
        "Request not succesful, expected 200, got: " + response.status
      );
...
const getHistoricWeather = () => {
  return fetch(`/weather/historical`, {
    method: "GET",
  }).then((response) => response.json());
};

//this function adds an eventlistener to the button read more at the weather forecast
document.addEventListener("DOMContentLoaded", () => {
  let readMoreBtn = document.getElementById("weatherReadMoreBtn");
  readMoreBtn.addEventListener("click", async () => {
    try {
      const response = await getHistoricWeather();
      alert(JSON.stringify(response));
    } catch (error) {
      console.error(error);
      // Handle any errors that occurred during the fetch request
      alert("Error occurred while fetching historic weather data");
    }
  });
});
...
function getDayName(dateStr, locale) {
  ...
  weekdays[0] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-04")
  ); // Søndag
  weekdays[1] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-05")
  ); // Mandag
  weekdays[2] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-06")
  ); // Tirsdag
  weekdays[3] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-07")
  ); // Onsdag
  weekdays[4] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-01")
  ); // Torsdag
  weekdays[5] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-02")
  ); // Fredag
  weekdays[6] = new Intl.DateTimeFormat(locale, { weekday: "long" }).format(
    new Date("1970-01-03")
  ); // Lørdag
...
const getWeather = async () => {
  ...
  for (let day in forecastData) {
    ...
    forecast.push({
      weekday: getDayName(forecastData[day].datekey, "da-DK")
        .substring(0, 3)
        .toUpperCase(),
    ...
    for (let col = 0; col < 7; col++) {
      ...
      if (row === 0) {
        ...
        let cellText = document.createTextNode(
          weather.forecast[col].temperature + "°"
        );
        cell.appendChild(cellText);
      }
